...
export function grokOgHeadTags({
  ...
  const imageHost = publicHost || "brunoberrutti.uy";
  const asset = resolveOgCardAsset(site, cwd) || "/og.jpg";
  const custom = Boolean(asset);
  let image = custom
    ? `https://${imageHost}${asset.startsWith("/") ? asset : `/${asset}`}`
    : `${ogServiceUrl()}/v1/card.png?host=${encodeURIComponent(imageHost)}&title=${encodeURIComponent(title)}`;
  ...
  if (banner) {
    const bannerUrl = `https://${imageHost}${banner.startsWith("/") ? banner : `/${banner}`}`;
